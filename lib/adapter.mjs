...
async function * findEntity (ctx) {
  const { adapter, hooks, intent } = ctx
  ...
  for (const hook of hooks.onFind) {
    await hook(ctx.argument, ctx)
  }

  const data = isFunction(adapter.findEntity)
    ? await adapter.findEntity(ctx.argument, ctx)
    : await adapter.find(ctx.argument, ctx.options)
...
async function * filterEntities (ctx) {
  ...
  if (isFunction(adapter.filterEntities)) {
    yield * adapter.filterEntities(ctx.argument, ctx)
  } else {
    yield * adapter.filter(ctx.argument, ctx.options)
  }
}
